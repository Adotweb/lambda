...
pub fn lex<'a>(text: &'a str) -> Vec<TokenType> {
    ...
    let id_match: Vec<String> = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_"
        .chars()
        .map(|x| x.to_string())
        .collect();

    let num_match: Vec<String> = "0123456789".chars().map(|x| x.to_string()).collect();
