    ...
    fn set(&mut self, key: String, new_value: Expression) -> Result<(), ()> {
        if let Some(value) = self.values.get_mut(&key) {
            *value = new_value;
            ...
            let _ = enclosing.borrow_mut().set(key.clone(), new_value.clone());
...
fn use_directive(
    ...
    match name.as_str() {
        ...
        "print_e" => {
            println!(
                "{}",
                evaluate(argument.clone(), enclosing.clone()).get_display()
            )
        }
        ...
        "print_u" => {
            println!(
                "{:#?}",
                evaluate(argument.clone(), enclosing.clone())
            )
        }

        _ => {
            let _ = enclosing.borrow_mut().set(name.clone(), argument.clone());
...
fn beta_reduce(
    ...
    match body {
        ...
        Expression::Grp { inner } => {
            Expression::Grp{
                inner : Box::new(beta_reduce(argument_name, argument, *inner, enclosing.clone()))
            }
        }
        Expression::Def {
        ...
        Expression::App { left, right } => {
            let new_left = evaluate(beta_reduce(
                argument_name.clone(),
                argument.clone(),
                *left,
                enclosing.clone(),
            ), enclosing.clone());
            let new_right = evaluate(beta_reduce(argument_name, argument, *right, enclosing.clone()), enclosing.clone());
...
fn apply(left: Expression, right: Expression, enclosing: Rc<RefCell<Environment>>) -> Expression {
    ...
    match left {
        ...
        Expression::Directive { name } => use_directive(name, right, enclosing.clone()),
        _ => left,
    }
}
...
pub fn evaluate(expr: Expression, enclosing: Rc<RefCell<Environment>>) -> Expression {
    match expr {
        ...
        Expression::App { left, right } => apply(*left, *right, enclosing.clone()),
        Expression::Grp { inner } => evaluate(*inner, enclosing.clone()),
        Expression::Directive { ref name } => {
            let stored = enclosing.borrow().get(name.to_string());

            if let Expression::None = stored{
                return expr
            }else {
                return stored
            }
